# Hongxian Huang Academic Website

A lightweight static academic website deployed through GitHub Pages.

## Production URL

- https://www.hongxianhuang.com/

## Main pages

- `index.html` — homepage and research profile
- `research/index.html` — publications, working papers, and abstracts
- `teaching/index.html` — teaching experience and interests
- `vitae/index.html` — education and experience from the master's level onward, awards, grants, and service
- `sitemap.xml` and `robots.txt` — search-engine discovery files

The portrait is stored at `assets/img/hongxian-huang-portrait.jpg`.

## Deployment

Upload the **contents of this folder** to the root of `hh1643/hh1643.github.io` and publish GitHub Pages from the `main` branch and repository root. The `.nojekyll` and `CNAME` files should remain in the root. The repository can continue to be `hh1643/hh1643.github.io`; the public site uses the custom domain.

## Current release

`v2026.09.15`

This release uses NYU Stern School of Business in the academic profile, removes the CV download and public PDF, and removes the Research in Progress section. Working papers omit journal-submission and review-status labels, and the Vitae page retains education and experience from the master’s level onward.
